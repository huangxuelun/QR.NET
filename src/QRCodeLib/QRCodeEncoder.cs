    ...
    public class QRCodeEncoder
    {
        ...
        public virtual bool[][] calQrcode(byte[] qrcodeData)
        {
            ...
            try
            {
                ...
                var resourceBytes = GetObject(fileName);
                using (MemoryStream memoryStream = new MemoryStream(resourceBytes))
                {
            ...
            for (int i = 0; i < 15; i++)
            {
                ...
                sbyte content = (sbyte)SByte.Parse(formatInformationArray[formatInformationValue].Substring(i, (i + 1) - (i)));
        ...
        private static byte[] GetObject(string fileName)
        {
            var fullPath = Path.Combine("..\\Resources", fileName);
            var resourceBytes = File.ReadAllBytes(fullPath);
