			...
			for (int y = 0; y < sqrtCenters; y++)
			{
				for (int x = 0; x < sqrtCenters; x++)
				{
					...
					if (y == 0)
					{
						...
						canvas.drawCross(centers[x][y], ColorFields.RED);
					}
					else if (x == 0)
					{
					...
					canvas.drawCross(centers[x][y], ColorFields.BLUE);
					canvas.drawLine(new Line(tmpPoint, centers[x][y]), ColorFields.LIGHTBLUE);
