    ...
    public class QRCodeSymbol
	{
		...
		public virtual String VersionReference
		{
			get
			{
				...
				return Convert.ToString(version) + "-" + versionReferenceCharacter[errorCollectionLevel];
			}
		...
		public virtual String MaskPatternRefererAsString
		{
			...
			get
			{
				String maskPattern = Convert.ToString(MaskPatternReferer, 2);
			...
			for (int y = 0; y < sideLength; y++)
			{
				for (int x = 0; x < sideLength; x++)
				{
					if (!(x == 0 && y == 0) && !(x == sideLength - 1 && y == 0) && !(x == 0 && y == sideLength - 1))
						if (Math.Abs(alignmentPattern[x][y].X - targetX) < 3 && Math.Abs(alignmentPattern[x][y].Y - targetY) < 3)
							return true;
				}
			}			
			return false;
